...
ylabel('v/(m per s)');
set(gca,'XLim',[0 2300])
title('A male Time Trail Specialist');
t
...
while (x < 11000)
    ...
    if(flag == 0)
        a = (F - (M*g*sin(alpha(x))) - mug*M*g*cos(alpha(x)) -0.6*Cd*FA*(v(t)*v(t)+vw*vw+2*v(t)*vw*cos(theta(x))))/M;
    else
        a = -v(t)/3*2;
    end
    v(t+1) = v(t)+a*dt;
